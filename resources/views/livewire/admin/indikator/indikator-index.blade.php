    ...
    <div class="flex flex-wrap items-center justify-between mt-3 gap-4">
        ...
        <div class="flex w-full flex-col gap-4 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
            <div class="flex gap-3">
                <div>
                    <a href="{{route('indikator.create')}}" wire:navigate type="button" class="btn btn-primary">
            ...
            <div class="relative">
                <input type="text" placeholder="Cari.." class="peer form-input py-2 ltr:pr-11 rtl:pl-11" wire:model.live="search">
                ...
                <tbody>
                    @forelse($indikators as $index => $indikator)
                        <tr>
                            ...
                            <td>{{ $indikator->aspek->nama }}</td>
                            <td>{{ $indikator->nama_indikator }}</td>
                            <td>
                                <div class="flex items-center gap-4">
                                    <a href="{{ route('indikator.edit', $indikator->aspek_id) }}" wire:navigate class="btn btn-sm btn-outline-primary whitespace-nowrap">Multi Ubah</a>
                                    <a href="{{ route('indikator1.edit', $indikator->id) }}" wire:navigate class="btn btn-sm btn-outline-primary">Ubah</a>
                                    <button type="button" wire:click.prevent="deleteContirmation({{ $indikator->id }})" class="btn btn-sm btn-outline-danger">
                                        Hapus
                                    </button>
                                </div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center text-muted">Belum ada data indikator</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
